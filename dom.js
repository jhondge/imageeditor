...
function make(node/*, ...instructions*/) {
	var instructions = Array.prototype.slice.call(arguments, 1);
	/*	Usage:
			...
			make(<falsy>, <instruction>*)				creates a document fragment

		The first argument is the node you want to modify/create, and the remaining arguments describe how you want to modify it.
		...
		There are five kinds of instructions:
			1. falsy
				Any instruction that is falsy doesn't do anything.
			...
			2. array
				...
				Again there are four kinds of elements:
					1. falsy
						Any element that is falsy is ignored.
					2. array
			...
			3. string
				Most of the time you do NOT want to use this. But sometimes it's useful.
	...
	*/
	...
	if (!node) {
		node = dom.doc.createDocumentFragment();
	} else if (typeof node === 'string') {
		node = dom.doc.createElement(node);
	}
	...
	function processChild(child) {
		if (!child) {
			...
			node.insertBefore(dom.doc.createTextNode(child), cursor);
